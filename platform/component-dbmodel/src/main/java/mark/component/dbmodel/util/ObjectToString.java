...
public final class ObjectToString {
    ...
    private static void appendHeader(final Object object,
            ...
            final StringBuilder buffer) {
        if (object != null) {
            buffer.append(indent(indent)).append(object.getClass().getName()).append('@').append(Integer.toHexString(System.identityHashCode(object))).append('[').append(Utility.NEWLINE);
        }
    }
    ...
    private static void appendObject(final Object object,
        ...
        if (Map.class.isAssignableFrom(objectClass)) {
            ...
            for (final Map.Entry mapEntry : mapEntries) {
                buffer.append(Utility.NEWLINE).append(indent(indent)).append(mapEntry.getKey()).append(": ").append(mapEntry.getValue());
            }
        } else if (Collection.class.isAssignableFrom(objectClass)) {
            for (final Iterator<?> iterator = ((Collection<?>) object).iterator(); iterator.hasNext();) {
            ...
            for (final Iterator<?> iterator = Arrays.asList((Object[]) object).iterator(); iterator.hasNext();) {
                final Object item = iterator.next();
                buffer.append(item);
                if (iterator.hasNext()) {
                    buffer.append(", ");
